      ...
      <div className="cf-content-aside">
        <div>
          <section className="cf-card cf-dashboard-hero">
            ...
            <div className="cf-continue">
              ...
              <h3>{current?.title ?? "故事的第一章"}</h3>
              <p>
                {data.activeTask
                  ? "有创作任务正在处理，可在任务中心查看。"
                  : "一章一章，让想象成为文字。"}
              </p>
              <Link className="cf-primary" to={link}>
                <PenLine size={18} />
                继续写作
              </Link>
          ...
          <div className="cf-suggestion-grid">
            <Link className="cf-card" to={link}>
              <CheckCircle2 />
              <strong>{current ? "本章已准备好" : "开启第一章"}</strong>
              <p>{current?.title ?? "先写下来，故事会慢慢展开。"}</p>
            </Link>
        ...
        <aside className="cf-side-stack">
          <section className="cf-card">
            <h2>待处理</h2>
            {[
              ["章节检查", data.pending.reviewIssues],
              ["AI 改写建议", data.pending.revisionProposals],
              ["设定变化", data.pending.canonChangeSets],
            ].map(([label, value]) => (
              <div className="cf-stat" key={label}>
                <strong>{label}</strong>
                <b>{value}</b>
              </div>
            ))}
