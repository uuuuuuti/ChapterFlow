...
刷新接口只负责生成待审版本和记录 `contentHash`，不会自动激活或批量覆盖旧卡片。若旧版本已有活动知识卡，刷新会复制出引用新来源版本的 `CANDIDATE` 卡片，活动卡仍保持不变；管理员确认来源和每张卡片后，才可以分别激活它们，并将原版本标为 `OUTDATED` 或 `SUPERSEDED`。引用非活动来源的卡片不能直接激活。发生解析失败、页面不可达或规则含义不确定时，保留旧版本、标出 `FETCH_FAILED`/未确认状态，并要求人工复核。
...
管理页面支持按阶段和关键词回看来源与卡片；刷新会尝试读取受限大小的官方页面，并把内容哈希写成 `CANDIDATE` 版本。网络失败会写入独立的 `FETCH_FAILED` 版本，同时保留原活动版本。只有确认 URL、版本和卡片内容后，管理员才应调用版本激活接口。
