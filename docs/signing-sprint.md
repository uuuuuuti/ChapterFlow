...
开篇检查只报告可复核信号，例如字数、段落长度、对话占比、人物/专有名词密度、连续解释段、重复段落和视角提示词，并尽量保留可回到原文的位置。它读取章节的最新可读正文：正式版本优先使用自动保存的作者草稿；空草稿或没有可读正文的章节仍会被标为缺失。信号不是质量判决，也不生成平台分数或签约概率。
...
新的结构化候选必须满足：作品包装至少 3 个候选，开篇计划必须恰好包含 3 个首章候选，并至少规划 3 个第一阶段章节；产品建议第一阶段按 10—20 章规划，但不把它写成平台硬规则。章节写作意图接受后只更新 Chapter Intent，不直接写入正文。测试模型只用于回归，不会加载到生产。

本地有真实模型配置时，可运行 `npm run test:real:signing-sprint -- --protocol=openai-responses` 验证固定的真实模型 Signing Sprint 场景：男性向都市悬疑“死者临终前七秒”，从空白项目依次完成方向、定位、定位复核、故事引擎、包装（3—5 个候选并选择）、开篇蓝图（前三章和第一阶段）、前三章正文夹具、开篇复核（问题带具体位置）、章节写作意图（正文版本数不变）、开篇检查和签约准备预检。脚本通过真实 HTTP 路由执行候选生成、显式接受和落地，并记录模型、来源引用、运行状态和清理后的摘要；缺少 API 配置或外部余额不足时会明确失败，不会用假响应冒充真实模型通过。
